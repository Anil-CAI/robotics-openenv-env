...
# ✅ EXISTING
BASE_URL = os.getenv("API_BASE_URL", "https://abito3423-robotics-openenv.hf.space")
...
# Safe init (won't crash if no key)
client = None
if HF_TOKEN:
...
# ✅ ADDED (REQUIRED LOG FORMAT START)
print("START")
...
while not done:
    ...
    # ✅ CHANGED LOG FORMAT (REQUIRED)
    print(f"STEP action={action} score={score:.2f}")

    if done:
        if tuple(obs["position"]) == tuple(obs["goal"]):
            print("🎯 Goal reached")
        else:
            print("⚠️ Goal unreachable")
        break
...
if difficulty == "easy":
    ...
    final_score = grade_hard(done, steps, 0)

# ✅ ADDED FINAL REQUIRED LOG
print(f"END final_score={final_score:.2f}")
